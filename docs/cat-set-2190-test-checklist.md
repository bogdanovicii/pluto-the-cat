...
- [ ] Bath Time budget: keep misting the same charmed enemy. Bath Time adds at most `SprayCharmMaxBonusSeconds` (6 s) to that charm however long it already was (a 10 s Wet Food Can charm tops out at 16 s, a 20 s Dinner Time charm at 26 s); the fourth mist adds nothing and logs nothing. Then re-charm the enemy with the can and mist it again: it should get a fresh 6 s budget. This last part is a genuine unknown, not a claim -- it depends on whether the engine reuses the same effect instance on a refresh, which could not be read from the stripped reference assembly. If the budget does not reset, say so; the fix is a reset on re-application rather than the duration-drift check.
